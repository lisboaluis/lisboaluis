# lisboaluis
<h1 align="center">Hi 👋, I'm Luis Henrique</h1>
<p align="center">
  Cloud Support Engineer passionate about data, automation, and cloud solutions.
</p>

---

💼 **About Me**

- 🎓 Degree in IT Management  
- ☁️ Currently working as a Cloud Support Engineer with focus on **Azure**, **Databricks**, **Microsoft Fabric**, and **DevOps**  
- 📊 Passionate about data engineering, automation, and artificial intelligence  
- 🧠 Currently studying for:  
  - **DP-700** (Fabric Data Engineer Associate)  
  - **Databricks Certified Data Engineer Associate**

---

🛠 **Technologies & Tools**

- **Cloud & Data:** Azure, Azure Data Factory, Microsoft Fabric, Synapse, Event Hub, Databricks  
- **Languages:** Python, SQL, KQL, PowerShell  
- **DevOps:** Azure DevOps, GitHub Actions, Docker  
- **Monitoring & Security:** Azure Monitor, Log Analytics, Wiz, Zabbix, Grafana  

---

📫 **Get in Touch**

- [LinkedIn](https://www.linkedin.com/in/luis-henrique-lisboa)  
- 📧 lisboaluis@icloud.com

---

⚡ *“Turning data into smart decisions.”*